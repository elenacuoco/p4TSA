...
namespace {
    ...
    public:
        ...
        void SetOp(unsigned newop) {
            op = newop;
        }

        void SetFuncNo(unsigned newno) {
            funcno = newno;
        }
        ...
        bool IsInvertedOriginal() const {
            return IsInverted() && !IsNegated();
        }

        bool IsNegatedOriginal() const {
            return !IsInverted() && IsNegated();
        }
...
#define CHECKCONSTNEG(item, op) \
    ...
    public:
        ...
        const SubTree& getp0() const {
            /*chk<1>();*/pcit tmp = GetBegin();
            return *tmp;
        }

        const SubTree& getp1() const {
            /*chk<2>();*/pcit tmp = GetBegin();
            ++tmp;
            return *tmp;
        }

        const SubTree& getp2() const {
            /*chk<3>();*/pcit tmp = GetBegin();
            ++tmp;
            ++tmp;
            return *tmp;
        }
        ...
        void Erase(const pit p) {
            data->args.erase(p);
        }

        SubTree& getp0() {
            /*chk<1>();*/pit tmp = GetBegin();
            return *tmp;
        }

        SubTree& getp1() {
            /*chk<2>();*/pit tmp = GetBegin();
            ++tmp;
            return *tmp;
        }

        SubTree& getp2() {
            /*chk<3>();*/pit tmp = GetBegin();
            ++tmp;
            ++tmp;
            return *tmp;
        }
        ...
        void OptimizeLogarithm() {
            ...
            if (GetOp() == cPow) {
                ...
                if (p0->IsImmed() && p0->GetImmed() == CONSTANT_E
                    ...
                    pit poslogpos;
                    bool foundposlog = false;
                    pit neglogpos;
                    bool foundneglog = false;
